...
{
    [RequireComponent(typeof(UnitEquipmentSlotUI))]
    public class EquipmentSlotUIInventorySelector : MonoBehaviour
    {
        [SerializeField] Image userSelectImage; // 유저가 선택했을 때 표시할 이미지
        private bool isSelect; // 유저가 선택했는지 여부
        [HideInInspector] public UnitEquipmentSlotUI unitEquipmentSlotUI;    // 장비 아이템 슬롯

        private void Awake()
        {
            unitEquipmentSlotUI = GetComponent<UnitEquipmentSlotUI>();
        }
